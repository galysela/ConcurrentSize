...
if len(sys.argv) == 1 or sys.argv[1] == '-h':
  print("Usage: python3 run_java_experiments_overhead.py <initSize> <ins>-<del> <workloadThreadsList> <sizeThreads> <num_warmup_repeats> <num_repeats> <runtime> <JVM memory size> <shouldRunMeasurements>")
  print("For example: python3 run_java_experiments_overhead.py 10000 3-2 \"[1,4]\" 0 1 1 1 1G T")
  exit(0)
  ...
  for ds in dataStructures:
    for workloadThreads in workloadThreadsList:
      ...
      if 'Size' in ds:
        ...
        sizeThreadsForDs = '0'
      cmd = cmdbase + workloadThreads + " " + sizeThreadsForDs + " " + str(repeats) + " " + runtime + " " + ds + " -ins" + ins + " -del" + rmv + " -initSize" + str(initSize) + " -prefill -file-build/data-trials" + str(i) + ".csv"
      if os.system(cmd) != 0:
        print("")
        exit(1)
...
def draw_graphs():
  ...
  graph_file_path = os.path.join(GRAPH_DIR, graph_name + "_%s_" + benchmark_name + ".png")
  graph.plot_overhead_graphs(results_file_path, graph_file_path, warmupRepeats)
  bars_graph_file_path = os.path.join(GRAPH_DIR, graph_name + "_bars_%s_" + benchmark_name + ".png")
  graph.plot_overhead_bars(results_file_path, bars_graph_file_path, warmupRepeats, False)
