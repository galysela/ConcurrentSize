...
if len(sys.argv) == 1 or sys.argv[1] == '-h':
  print("Usage: python3 run_java_experiments_per_size.py <initSizeList> <ins>-<del> <workloadThreads> <sizeThreads> <num_warmup_repeats> <num_repeats> <runtime> <JVM memory size> <shouldRunMeasurements>")
  print("For example: python3 run_java_experiments_per_size.py \"[100000,1000000,10000000]\" 3-2 4 1 1 1 1 1G T")
  exit(0)
...
initSizeList = sys.argv[1][1:-1].split(',')
...
def draw_graphs():
  ...
  graph.plot_per_size_graph(results_file_path, os.path.join(GRAPH_DIR, graph_name + "_sizeThreads_" + benchmark_name + ".png"), warmupRepeats, True)
  graph.plot_per_size_graph(results_file_path, os.path.join(GRAPH_DIR, graph_name + "_sizeThreads_" + benchmark_name + "_others.png"), warmupRepeats, False)


graph_name = "per-size"
benchmark_name = str(ins) + "ins-" + str(rmv) + "rem_" + str(workloadThreads) + "workloadThreads_" + str(sizeThreads) + "sizeThreads"
